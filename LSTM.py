...
def collate_fn(batch):
    ...
    # Avoid sorting if batch size is 1 (no need to sort a single sequence)
    if len(batch) > 1:
    ...
    # Ensure lengths is at least 1D (needed for batch_size=1 case)
...
plt.ylabel('Validation Accuracy')
plt.title('Validation Accuracy vs. Learning Rate (LSTM)')
plt.legend()
plt.grid()
...
plt.xlabel('Learning Rate')
plt.ylabel('Total Training Time (seconds)')
plt.title('Training Time vs. Learning Rate (LSTM)')
plt.legend()
plt.grid()
plt.subplots_adjust(bottom=0.15, left=0.15)  # Prevent label overlap
